"""
EvaGiantTagger — Danbooru-v4 EVA-Giant tagger (animetimm).

Thin concrete :class:`Dbv4Tagger` for
``animetimm/eva_giant_patch14_560.dbv4-full``. All load / preprocess / vocab /
decode behaviour comes from :class:`Dbv4Tagger`; only the identity and files
are declared here (the input size is sourced from the repo's preprocess.json).
"""
...
class EvaGiantTagger(Dbv4Tagger):
    """EVA-Giant Danbooru-v4 tagger (animetimm).
    ...
    folder (``Models/eva_giant_patch14_560.dbv4-full``), the file paths, and the
    ...
    * input side    : 560 (the ``test`` ``resize`` step)
    * normalization : ImageNet mean/std ([0.485, 0.456, 0.406] / [0.229, 0.224, 0.225])
    ...
    2. **Load + metadata** -- :meth:`Dbv4Tagger._load` builds a transformers
       ...
       timm ``eva_giant_patch14_560`` backbone) and :meth:`Dbv4Tagger._ensure_metadata`
       parses ``preprocess.json`` (560 + ImageNet), ``categories.json``
    ...
    3. **Preprocess** -- :meth:`Dbv4Tagger._preprocess_image`: a *full padded
       ...
       cropped) -> bicubic resize to 560 -> /255 + ImageNet -> CHW float32.
    ...
    model_id = "animetimm/eva_giant_patch14_560.dbv4-full"
